...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <a href="{{ route('intouch.beheer.afstanden.create') }}" class="btn btn-primary">Nieuwe afstand</a>
    @endcan
</div>
            ...
            <tbody>
                @forelse($distances as $d)
                    <tr>
                        ...
                        <td class="text-end">
                            @can('afstanden_edit')
                            <a href="{{ route('intouch.beheer.afstanden.edit', $d) }}" class="btn btn-sm btn-outline-primary">Bewerken</a>
                            @endcan
                            @can('afstanden_delete')
                            <form method="post" action="{{ route('intouch.beheer.afstanden.destroy', $d) }}" class="d-inline" onsubmit="return confirm('Weet je het zeker?');">
                                @csrf
                                @method('DELETE')
